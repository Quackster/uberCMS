...
{		 
	echo '<div id="active-habbo-data-' . $i . '" class="active-habbo-data"> 
		<div class="active-habbo-data-container"> 
			...
			Habbo created on: ' . $randomHabbo['account_created'] . '
				<p class="moto">' . clean($randomHabbo['motto']) . '</p> 
		</div> 
	</div>                
	<input type="hidden" id="active-habbo-url-' . $i . '" value="%www%/home/' . clean($randomHabbo['username']) . '"/> 
	<input type="hidden" id="active-habbo-image-' . $i . '" class="active-habbo-image" value="http://www.habbo.co.uk/habbo-imaging/avatarimage?figure=' . clean($randomHabbo['look']) . '&direction=4&head_direction=4" />';
